...
def verifier_apis(**context):
    ...
    for nom, url in apis.items():
        try:
            ...
            raise ValueError(f"Erreur réseau lors de la vérification de l'API {nom} : {str(e)}")
...
def collecter_meteo_regions(**context):
    ...
    for region, coords in REGIONS.items():
        ...
        ensoleillement_h = json_data["daily"]["sunshine_duration"][0] / 3600
        vent_kmh = json_data["daily"]["wind_speed_10m_max"][0]
        ...
        logging.info(f"{region}: {ensoleillement_h:.2f}h d'ensoleillement, vent max {vent_kmh:.1f} km/h")
...
def collecter_production_electrique(**context):
    ...
    # À compléter :
    # 1. Appeler l'API et récupérer response.json()["results"]
    # 2. Initialiser un dict intermédiaire pour accumuler les valeurs par région :
    #    accumulation = {region: {"solaire": [], "eolien": []} for region in REGIONS}
    # 3. Pour chaque enregistrement :
    #    - Récupérer enregistrement["libelle_region"]
    #    - Si la région est dans REGIONS, ajouter les valeurs solaire et eolien
    #      (remplacer None par 0.0 avec : valeur = enregistrement.get("solaire") or 0.0)
    # 4. Calculer la moyenne par région (sum / len de la liste)
    # 5. Construire et retourner le dict final {region: {solaire_mw, eolien_mw}}
    production = {}
    # ... votre code ici ...
    return production
...
def analyser_correlation(**context):
    ...
    logging.info(f"{nb_alertes} région(s) en alerte détectée(s)")
    return alertes
